...
@pytest.mark.asyncio
async def test_async_update_data_does_not_emit_warm_water_power_on_sc2(dummy_coordinator, monkeypatch):
    # SC2 devices skip the warm_water_power register (SC3-only), so the coordinator should not add it via derivation.
    dummy_coordinator.config_entry.data[DEVICE_VERSION] = 2
    dummy_coordinator.supported_version = 2

    warm_water_temp = DummyRegister("warm_water_temp_s2", 33025, 0, 0, poll_rate=0, poll_time=0, reg=1, multiplier=0.1)
    cold_water_temp = DummyRegister("cold_water_temp_s15", 33038, 0, 0, poll_rate=0, poll_time=0, reg=1, multiplier=0.1)
    warm_water_volume_flow = DummyRegister("warm_water_volume_flow_s18", 33041, 0, 2, poll_rate=0, poll_time=0, reg=1, multiplier=1.0)
    warm_water_power = DummyRegister("warm_water_power", 33549, 0, 1, poll_rate=2, poll_time=0, reg=2, multiplier=1.0)
    ...
    async def read_registers(address, count):
        values = {
            33025: [500],  # 50.0 °C
            33038: [120],  # 12.0 °C
            33041: [60],   # raw SC2 S18 value (processed in entity layer)
            33549: [27],
        }
        return DummyModbusResponse(values[address])
    ...
    assert "warm_water_power" not in data
